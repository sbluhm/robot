...
//    gpioSetMode(19, PI_INPUT); // Speed Pulse
  ...
  void set_motor_values(double left, double right)
  {
    ...
    const double MIN_SPEED = 0.027;
    const double A = 227.638572;
    const double B = 15.904997;
    const double C = 39.948085;
...
//        power = static_cast<int>(round( abs(left * RADIUS) + MOTOR_SHIFT ) * MOTOR_ROC * 10000 );
	power = static_cast<int>( ( A * exp(-B * abs(left)*RADIUS) + C ) *abs(left)*RADIUS* 10000  );
...
//        power = static_cast<int>( round(abs(right * RADIUS) + MOTOR_SHIFT ) * MOTOR_ROC * 10000 );
        power = static_cast<int>(( A * exp(-B * abs(right)*RADIUS) + C ) *abs(right)*RADIUS* 10000 );
